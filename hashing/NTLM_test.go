...
func TestParseNTLMSSP_Auth(t *testing.T) {
	...
	if !bytes.Equal(lmResponceExpected, obj.Lan_Manager_Response) {
		t.Errorf("lmResponce is not correct.\nExpect: %#v\nGot: %#v", lmResponceExpected, obj.Lan_Manager_Response)
	}
	...
	if !bytes.Equal(NTResponceExpected, obj.NTLM_Response) {
		t.Errorf("NTResponceExpected is not correct.\nExpect: %#v\nGot: %#v", NTResponceExpected, obj.NTLM_Response)
	}
	...
	if !bytes.Equal(expectedDomain, obj.Domain_name) {
		t.Errorf("expectedDomain is not correct.\nExpect: %#v\nGot: %#v", expectedDomain, obj.Domain_name)
	}
	...
	testEqual("username", expectedUserName, obj.User_name, t)
	...
	testEqual("Hostname", expectedHostName, obj.Host_name, t)

	expectedSessionKey := []byte{0xf2, 0x6d, 0x0d, 0x78, 0xdf, 0x40, 0x14, 0xcc, 0xbf, 0x0d, 0x2e, 0x7f, 0xd3, 0x4f, 0x25, 0xf6}

	testEqual("sessionkey", expectedSessionKey, obj.Session_Key, t)

	expectedFlags := []byte{0x15, 0x82, 0x88, 0xe2}

	testEqual("flags", expectedFlags, obj.Negotiate_Flags, t)
...
func Test_getHumanNegoTargetInfo(t *testing.T) {
	...
	obj, err := getHumanNegoTargetInfo(testcase)
	if err != nil {
		t.Fatal(err)
	}
